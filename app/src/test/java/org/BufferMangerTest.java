...
// //Checks if the buffer capacity, number of free frames, total pages and if
// the buffer is empty
...
// //Test if the createPage method works correctly by seeing if the buffermanger
// correctly updates the number of free frames, total pages and if the page is
// dirty
...
// //Checks if the page is dirty, pin count, total pages and free frames are set
// correctly (pin count is 1, total pages is i and free frames is 10-i)
// assertEquals(test.getPageMetaData(i-1).getPinCount(), 1, "Pin count should be
// is 1");
// assertEquals(test.getPageMetaData(i-1).isDirty(), true, "Page is supposed to
// be dirty");
// assertEquals(i, test.getTotalPages(), "Total pages is " + i);
// assertEquals(10-i, test.getFreeFrames(), "Free frames is " + (10-i));
// }
// // The BufferManager should not be able to create more pages as it has
// reached its capacity and all current pages are pinned
// Page page = test.createPage();
...
// //Test if the createPage method works correctly in the case where the buffer
// manager is full and some pages are unpinned
...
// //Checks if the page is dirty, pin count, total pages and free frames are set
// correctly (pin count is 1, total pages is 10+i and free frames is 10-i)
// assertEquals(test.getPageMetaData(9+i).getPinCount(), 1, "Pin count should be
// is 1");
// assertEquals(test.getPageMetaData(9+i).isDirty(), true, "Page is supposed to
// be dirty");
...
// // Test the GetPage method in the case where the page is not in the buffer
// pool
...
// // The manager should not be able to return a page as it has not created any
// pages with that id
// Page page = test.getPage(58);
// assertNull(page, "Fetched page is not null");
// }

// // Test the GetPage method in the case where the page is in the buffer pool
// or in memory
...
// // Checks if the page is dirty, pin count, total pages and free frames are
// set correctly (pin count is 1, total pages is i and free frames is 3-i)
// assertEquals(test.getPageMetaData(i).getPinCount(), 1, "Pin count should be
// is 1");
// assertEquals(test.getPageMetaData(i).isDirty(), true, "Page is supposed to be
// dirty");
...
// //Check if the pincount has gone up to 2 and the page is still dirty, and
// still in the cache
// assertEquals(test.getPageMetaData(i).getPinCount(), 2, "Pin count should be
// is 2");
...
// test.unpinPage(i);
// test.unpinPage(i);
// assertEquals(0, test.getPageMetaData(i).getPinCount(), "Pin count should be
// is 0");
// }
...
// assertNotNull(page, "Page is created and not null");
// assertTrue(test.existsInCache(i - 3) == -1, "page " + (i-3) + " should not be
// in cache");
// }

// // Verifying that page 0 and 1 are not in the buffer pool and LRU cache is
// correct
// assertTrue(test.existsInCache(0) ==-1, "page " + 0 + " should not be in
// cache");
// assertTrue(test.existsInCache(1) ==-1, "page " + 1 + " should not be in
// cache");
...
// assertTrue(test.getPageMetaData(2).getPinCount() == 0, "Pin count should be
// is 0");
// assertTrue(test.getPageMetaData(3).getPinCount() == 1, "Pin count should be
// is 1");
...
// // Checks if the page is dirty, pin count, total pages and free frames are
// set correctly (pin count is 1, total pages is 5 and free frames is 2)
// assertNotNull(page, "Fetched page should not be null");
// assertEquals(test.getPageMetaData(0).getPinCount(), 1, "Pin count should be
// is 1");
// assertEquals(test.getPageMetaData(0).isDirty(), false, "Page isn't supposed
// to be dirty");
// assertTrue(test.existsInCache(2) == -1, "page " + 2 + " should not be in
// cache");
...
// // Test if MarkDirty method works correctly by seeing if the buffermanger
// correctly updates the page as dirty
...
// assertTrue(test.getPageMetaData(pageId).isDirty(), "Page is marked as
// dirty");
...
// assertFalse(test.getPageMetaData(0).isDirty(), "Page is not marked as
// dirty");
...
// // Test unpinPage method in the case where the page is in buffer pool or in
// memory
...
// assertEquals(0, test.getPageMetaData(pageId).getPinCount(), "Page pin count
// is 0 after unpinning");
// }

// //Pin page 5 times
// for (int i = 0; i < 5; i++) {
// test.getPage(pageId);
// assertEquals(i+1, test.getPageMetaData(pageId).getPinCount(), "Page pin count
// is 0 after unpinning");
// }
...
// for (int i = 0; i < 5; i++) {
// test.unpinPage(pageId);
// assertEquals(5-i-1, test.getPageMetaData(pageId).getPinCount(), "Page pin
// count is 0 after unpinning");
// }
